...
__constant float EPSILON = 0.0000000001f; /* required to compensate for limited float precision */
...
bool intersect_octree(
	...
	float3 scaledDir = normalize(newRay.dir / (octrees[currOctreeIndex].max - octrees[currOctreeIndex].min));
	...
	for (int i = 0; i < object_count; i++) {
		if (i != hit.object && objects[i].light) {
			...
			if (INTERVAL == LIGHTLIKE) {
				...
				/*
				float4 objSimulPlane = normalize(transformPoint4D(objects[hit.object].InvLorentz, (float4)(1, 0, 0, 0)));
				float4 origin_ObjFrame = objects[hit.object].stationaryCam;

				float4 simulPlane = normalize(transformPoint4D(objects[i].InvLorentz, (float4)(1, 0, 0, 0)));
				float4 origin_LightFrame = transformPoint4D(objects[i].Lorentz, (float4)(hitPos.x, 0, 0, 0));
				float dist = dot(simulPlane, origin_LightFrame);

				float4 lightPos_LightFrame = (float4)(0, objects[i].M[0].w, objects[i].M[1].w, objects[i].M[2].w);
				lightPos_LightFrame.x = dot(simulPlane * dist - lightPos_LightFrame, simulPlane) / simulPlane.x;
				float4 lightPos = transformPoint4D(objects[i].InvLorentz, lightPos_LightFrame);
				float4 lightDir = lightPos - hitPos;
				float3 unitLightDir3_ObjFrame = normalize(lightDir.yzw);
				float3 newNorm = (float3)(
				...
				*/
				...
				if (dot(hit.normal, unitLightDir3) > 0) {
					Ray4D newRay;
					newRay.dir = (float4)(0, unitLightDir3);
					newRay.origin = hitPos;
